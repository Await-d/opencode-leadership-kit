...
>
> **重要说明**：OpenCode 官方并不内建 `Prometheus`。如果你已经安装了 **oh my opencode**，则应优先把本套件视为**兼容复用该插件中的 `Prometheus` 角色契约**；本仓库内的 `prometheus.md` 则作为本地后备实现，用来保证在没有插件时仍可运行。
...
@frontend-ui-ux-engineer 强化支付页的 UI/UX 方案
@document-writer 把当前方案整理成对外可读的文档
...
- 插件角色：`sisyphus / prometheus / atlas / hephaestus / librarian / oracle / frontend-ui-ux-engineer / document-writer / metis / momus ...`
...
| UI/UX 设计强化 | `frontend-ui-ux-engineer` |
| 文档整稿 / 文案打磨 | `document-writer` |
...
2. 本套件中的 `prometheus.md` 作为后备与兼容实现
